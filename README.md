# 教學訓練計畫主持人工作坊

依據「教學訓練計畫主持人工作坊.docx」製作的繁體中文視覺化網頁，支援填寫、確認資料及送至 Google Sheets。

## 檔案

- **dashboard.html**：報名現況儀表板，呈現累計、參與方式、機構數、職類分布、主持人身分與最近 7 日趨勢。
- **index.html**：紫色漸層、emoji、響應式單檔網頁，內含 HTML／CSS／JavaScript。
- **apps-script/Code.gs**：Google Apps Script 收件程式，已設定指定試算表與「工作坊報名資料」分頁。
- **apps-script/headers.tsv**：可整行貼到 A1 的 10 欄表頭。
- **[apps-script/SETUP.md](apps-script/SETUP.md)**：初始化、授權、部署、網頁設定及收件驗證步驟。
- **tests/**：Google 服務模擬測試與前端送出流程測試，無外部測試套件。

## 啟用收件

index.html 的 **APPS_SCRIPT_URL** 已填入提供的 Apps Script /exec 部署網址，網頁可確認資料並送出。主辦單位仍須依 SETUP.md 核對表頭與部署權限，並驗證實際收件。

網頁可直接開啟查看；正式報名請使用 HTTPS 網站並完成實際收件驗證。Google Sheets 網址不能直接當成送出網址。此專案未自動部署 Apps Script，也未寫入線上試算表。

## 名額上限

本活動限額 **4 名**。報名頁與儀表板在統計達 4 筆時顯示「額滿」，報名欄位及送出按鈕停用。報名頁開啟時及可見分頁每 30 秒檢查一次名額，也可手動重新確認。檢查失敗時暫停新報名；曾確認額滿則維持關閉。

**必須將本次最新版 Code.gs 更新至既有 Apps Script 部署，伺服器才會正式拒絕超額請求。** 僅 push GitHub 不會更新 Apps Script。新版在同一把 ScriptLock 內執行去重、名額檢查及寫入，第 4 筆可收件，第 5 筆回覆 FULL。既有報名編號重試即使額滿仍回覆原收件結果，且不新增資料。舊後端未確認啟用名額限制時，更新後的報名頁不會開放新報名。

名額沿用儀表板的計數規則：有姓名的列，以非空報名編號去重，空編號列各計一筆；不依 Email 判定自然人。同一人以不同編號報名會占用不同名額。

## 資料與行為

保留原文件日期 2026/01/25、時間 9:00–16:00、地點、聯絡資訊與 12 個職類選項。所有 8 個填寫欄位為必填。確認送出後，資料寫入指定試算表，新增台北時間及報名編號。

只有收到後端成功回覆才顯示收件成功；網路失敗可在同頁保持相同資料重試，沿用編號防止重複寫入。沒有瀏覽器持久儲存，後端保留電話前導零並防止輸入變成公式。

## 報名現況儀表板

直接開啟 **dashboard.html** 即可查看目前統計，報名頁也已加入入口。內建快照擷取於 **2026/09/07 22:29:26（台北時間）**：4 筆報名、實體 2 筆、線上 2 筆、4 個機構，目前擔任計畫主持人 2 筆。

儀表板開啟時讀取一次統計，亦可按「更新資料」。**即時讀取需要將本次更新的 Code.gs 貼回 Apps Script 並更新既有部署至新版本**；詳細操作見 [儀表板部署說明](apps-script/SETUP.md#報名現況儀表板)。原部署若只回傳服務識別，頁面會保留帶有擷取時間的快照，不會宣告已更新。

GET action=dashboard 僅回傳彙總；網站、快照與公開端點不包含報名者姓名、Email、電話、機構名稱或報名編號。統計有姓名的列，以非空報名編號去重；空編號列各計一筆。機構名稱僅去除首尾空白後去重計數。未知選項列入「未分類」；無法辨識日期者計入總數但不列入每日趨勢。

## 測試

```sh
node --test tests/*.test.cjs
```

測試不需連線、不使用真實個資、不寫入 Google Sheets。通過本地測試不代表 Apps Script 已完成部署；上線後請依 SETUP.md 驗證實際收件。
